...
import { BizDialogForQueryComponent } from '../bizpage/bizdialog.query.component';
...
export class BizDialogService {
     ...
     * 静态框，点击蒙层不允许关闭
     */
    modal(comp: any, params?: any, size: 'sm' | 'lg' | '' | number = 'lg', options?: any): Observable<any> {
        return this.open(comp, params, size, Object.assign({
            maskClosable: false
        }, options));
    }

    simpleModal(dialogUrl:string, params:any, size: 'sm' | 'lg' | '' | number = 'lg'){
        //debugger;
        let options = {};
        let dialogParams = {};
        
        dialogParams['dialogUrl'] = dialogUrl;
        dialogParams['dialogParams'] = params;        
        return this.open(BizDialogForQueryComponent, dialogParams, size, Object.assign({
            maskClosable: false
        }, options));
