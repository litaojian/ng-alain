...
export class BizQueryComponent extends BaseListComponent implements OnInit, AfterViewInit, AfterViewChecked, AfterContentInit, AfterContentChecked {

	@ViewChild("container", { read: ViewContainerRef })
	container: ViewContainerRef;
	bizQueryService:BizQueryService;
	dialogService:BizDialogService;
	pagePath:string;
	methodContainer:any;
	isOK:string;
	location: Location;		
	...
	constructor(
		...
		this.dialogService = injector.get(BizDialogService);
	...
	compileModuleAndComponent(pageName:string, pageType:string, htmlContent:Promise<Object>) {
		...
		//var greeter = Object.create(this.context["Greeter"].prototype);
		// var greeter = new window["Greeter"]();
	...
	ngAfterViewInit() {
		//console.log("bizQuery afterViewInit");
	}
	...
	showModalDialog(url: string, params: Object): boolean {
		//alert("debug:" + url);
		console.log("showModalDialog............")
		let size;
		if (params && params['dialogWidth']){
			size = params['dialogWidth'];
		}
		this.dialogService.simpleModal(url, params, size);
		return false;
	}  
}
