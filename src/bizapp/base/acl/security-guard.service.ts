...
export class SecurityGuardService extends BaseService implements CanActivate, CanActivateChild, CanLoad {
  ...
  constructor(injector:Injector,private router: Router, private location: Location, private ssoClientService:SsoClientService, private userPermissionService:UserPermissionService) {
    super(injector);
  }
  ...
  processTask(route: ActivatedRouteSnapshot, state: RouterStateSnapshot, cmd:string){
    ...
    }else if (url != null && url.startsWith("/sso/error")){
      return Promise.resolve(true);
    }
