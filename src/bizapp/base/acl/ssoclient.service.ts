 ...
 */
  buildLoginUrlForSSO(backtoUrl:string) {
    ...
    if (ssoServerUrl != null) {
      ...
      let url = `${ssoLoginUrl}?client_id=${this.clientId}&response_type=${response_type}&redirect_uri=${redirect_uri}&logout_uri=${logout_uri}`;
      //
  ...
  verifySignature(content, signature, pType): boolean {
    ...
    //debugger;
    if (pType == 2){
      content = content + "," +  enc.Base64.stringify(enc.Utf8.parse(this.clientId));
      //console.log(content);
    }
